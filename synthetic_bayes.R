...
for (g in 1:G){
  for ( b in 1:B){
    ...
    Y[[(g-1)*B + b]] = S[[(g-1)*B + b]] %*% Factors_blocks[[(g-1)*B + b]] + matrix(rnorm(Time * (N), mean = 0, sd=1/sqrt(Time)), 
                                                                                   Time, N)
    Y.scaled[[(g-1)*B + b]]  = scale(Y[[(g-1)*B + b]])
  }
}
...
for (g in 1:G){
  for ( b in 1:B){
    ...
    data2 <- list(N = N, T = Time, K= 20, X = Y.scaled[[(g-1)*B + b]], 
                  Structural_precision= Lap)
    stan.fit.vb <- vb(m, data = data2, algorithm = "meanfield")
    
    
    Loadings.vb[[(g-1)*B + b]] <- apply(rstan::extract(stan.fit.vb,"Loadings")[[1]], c(2,3), mean)
    A_tilde1.vb[[(g-1)*B + b]] <- apply(rstan::extract(stan.fit.vb,"A_tilde")[[1]], c(2,3), mean)
    A1.vb[[(g-1)*B + b]] <- apply(rstan::extract(stan.fit.vb,"A")[[1]], c(2,3), mean)
    ...
    ########## SAVE EVERYTHING #################
    sv_file = paste('~/Dropbox/Bayesian_ICA/synthetic/results/', save_folder,'_vb.RData', sep='')
    save.image(sv_file) 
    
    initf1 <- function() {
      list(A_tilde=A_tilde1.vb[[(g-1)*B + b]] , mu_A0 = mu_a0.vb[[(g-1)*B + b]], D=D1.vb[[(g-1)*B + b]], S=S1.vb[[(g-1)*B + b]]  ,
           Lambda = Lambda.vb[[(g-1)*B + b]],    sigma_noise_inv=sigma_noise_inv.vb[[(g-1)*B + b]])
    } 
    ...
    A1.stan[[(g-1)*B + b]] <- apply(rstan::extract(stan.fit,"A")[[1]], c(2,3), mean)
